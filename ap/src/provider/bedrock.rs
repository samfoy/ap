...
mod tests {
    ...
    // ── Step 2: system_prompt threading ──────────────────────────────────────

    #[test]
    fn provider_passes_system_prompt_to_bedrock() {
        let messages = vec![Message::user("Hello")];
        let body = BedrockProvider::build_request_body(&messages, &[], Some("be helpful"));
        assert_eq!(body["system"], "be helpful", "system field must contain the system prompt");
    }

    #[test]
    fn provider_no_system_prompt_omits_field() {
        let messages = vec![Message::user("Hello")];
        let body = BedrockProvider::build_request_body(&messages, &[], None);
        assert!(body["system"].is_null(), "system field must be absent when None");
    }
}
